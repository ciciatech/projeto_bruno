...
| `execucao_orcamentaria/ce/siof_obras_consolidado.csv` | variável | 2015–2026 | Ceará (obras) |
| `execucao_orcamentaria/al/transparencia_al_consolidado.csv` | 2.065 | 2015–2025 | Alagoas |
| `execucao_orcamentaria/pi/transparencia_pi_*.csv` | 250 | 2015–2025 | Piauí |

**Colunas CE (SIOF — relatório 101, por Secretaria):** `Código`, `Descrição`, `Lei`, `Lei + Cred.`, `Empenhado`, `Pago`, `% Emp.`, `% Pago`, `ano`, `mes`

**Colunas CE (SIOF Obras — relatório 110, elemento 51):** Mesma estrutura, filtrado por grupo 44 (Investimentos) + elemento 51 (Obras e Instalações). Contém hierarquia secretaria → órgão → programa → PA → região.

**Filtros disponíveis no SIOF-CE:**

| Filtro | Valores |
|--------|---------|
| Relatórios | 101 (Secretaria), 102 (Secretaria e Fonte), 120 (Secretaria e Região), 105 (Sec/Órgão/Class.), 110 (Sec/Órgão/Prog/PA/Região) |
| Regiões CE | 14 regiões (Cariri, Centro Sul, Grande Fortaleza, ..., Vale do Jaguaribe) |
| Elementos de despesa | 51 (Obras e Instalações), 52 (Equipamentos e Material Permanente) |

**Colunas AL:** `dotacao_inicial`, `dotacao_atualizada`, `empenhado`, `liquidado`, `pago`, `unidade_gestora`, `cod_ug`, `funcao`, `ano`

**Colunas PI:** `ano`, `cod_ug`, `unidade_gestora`, `funcao`, `empenhado`, `pago`, `liquidado`, `data`, `favorecido` (32 colunas no formato original)
...
| `execucao_orcamentaria/ce/siof_ce.csv` | 480 | Ceará 2015–2026 (consolidado por secretaria) |
| `execucao_orcamentaria/ce/siof_obras_secretaria.csv` | variável | Ceará — obras por secretaria/ano |
| `execucao_orcamentaria/ce/siof_obras_regiao.csv` | variável | Ceará — obras por região/ano (14 regiões) |
| `execucao_orcamentaria/pi/transparencia_pi.csv` | 250 | Piauí 2015–2025 |

**Colunas `siof_obras_regiao.csv`:** `ano`, `regiao`, `empenhado`, `pago`, `dotacao`, `n_acoes`
...
- `bolsa_familia/nordeste/bolsa_familia_uf_mensal.csv` (opcional)
- `bolsa_familia/nordeste/bolsa_familia_portal_transparencia.csv` (opcional)
...
| Fonte | Status | Achado |
|-------|--------|--------|
| `bacen_sgs_wide.csv` | OK | 132 registros, 5.13% missing (PIB trimestral) |
| `caged_antigo_raw` | Alerta | 117 combinações período-UF ausentes (13 meses corrompidos) |
| `caged_novo_raw` | OK | 648 registros, zero nulos, zero lacunas |
| `rais_vinculos_raw` | Alerta | anos 2019/2020 ausentes; `remuneracao_media` com nulos; 25 combinações ausentes |
| `siconfi_dca_raw` | Alerta | 2025 ausente (defasagem de publicação) |
| `bolsa_familia_*` | Alerta | Arquivos opcionais não encontrados (sem API key) |
| `execucao_orcamentaria_*` | Alerta | Arquivos processados opcionais não encontrados |
| `painel_tese_bimestral` | OK | 594 linhas, 37 colunas, 9.69% missing total |
...
### 3.1 Variáveis do MODEL_RULES (8 regras definidas em `preparacao_modelo.py`)

| Variável | Fonte | Agregação bimestral | Deflação | Unidade |
|----------|-------|---------------------|----------|---------|
| `saldo` | CAGED | soma mensal (fluxo) | Não | postos de trabalho |
| `credito_PF_nordeste_real` | BACEN | último valor (estoque) | IPCA | R$ dez/2025 milhões |
| `credito_PJ_nordeste_real` | BACEN | último valor (estoque) | IPCA | R$ dez/2025 milhões |
| `resultado_primario_real` | RREO | acumulado no período | IPCA | R$ dez/2025 |
| `dcl_real` | RGF | repetição quadrimestral | IPCA | R$ dez/2025 |
| `investimento_publico_real` | DCA | repetição anual | IPCA | R$ dez/2025 |
| `transferencias_federais_real` | RREO | diferença acumulada anual | IPCA | R$ dez/2025 |
| `rais_vinculos_ativos` | RAIS | repetição anual (estoque) | Não | vínculos formais |

### 3.2 Mapeamento completo

| Papel | Variável | Arquivo processado | Frequência | Status |
|-------|----------|--------------------|------------|--------|
| **Dependente** | Emprego formal (fluxo) | `model_ready/caged_bimestral.csv` | Bimestral | **Completo 2015–2025** |
| **Dependente** | Emprego formal (estoque) | `model_ready/rais_bimestral.csv` | Anual→Bimestral | Parcial (6 de 8 anos) |
| **Explicativa** | Crédito PF NE (real) | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Explicativa** | Crédito PJ NE (real) | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Controle** | IBCR-NE | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Controle** | IBC-Br | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Controle** | SELIC | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Controle** | IPCA | `model_ready/bacen_bimestral.csv` | Bimestral | Completo |
| **Controle** | Resultado Primário | `model_ready/resultado_primario_bimestral.csv` | Bimestral | Completo (9 UFs) |
| **Controle** | DCL | `model_ready/dcl_bimestral.csv` | Quadrimestral→Bimestral | Completo |
| **Controle** | Investimento público | `model_ready/investimento_publico_bimestral.csv` | Anual→Bimestral | Completo |
| **Controle** | Transferências federais | `model_ready/transferencias_bimestrais.csv` | Bimestral | Completo |
| **Controle** | Bolsa Família | `bolsa_familia/nordeste/bolsa_familia_uf_mensal.csv` | Mensal | Condicional à API key |
| **Painel final** | Base model-ready | `model_ready/painel_tese_bimestral.csv` | Bimestral | **594 linhas, 37 colunas** |
...
## 5. PENDÊNCIAS (pipeline NE legado)
...
## 6. PIPELINE REGIONAL CE — pedido Prof. Paulo (abr/2026)

Pipeline paralela cobrindo município → 14 regiões de planejamento da SEPLAG/IPECE,
em granularidade mensal. Ver `docs/README.md` seção "Pipeline regional CE" para o
roteiro de execução.

### Variáveis do painel `painel_regional_ce_mensal.csv`

Chave: `(regiao_codigo, regiao_nome, ano, mes)`. 14 × 132 = **1.848 linhas** previstas.

| Tipo | Variável | Coluna(s) | Fonte | Observações |
|------|----------|-----------|-------|-------------|
| Endógena | Saldo de emprego | `caged_saldo` | `caged_municipal` | Soma admissões − desligamentos (CAGED) |
| Endógena | Admissões | `caged_admissoes` | `caged_municipal` | Total CAGED |
| Endógena | Desligamentos | `caged_desligamentos` | `caged_municipal` | Total CAGED |
| Endógena | Salário médio | `salario_medio` | `caged_municipal` | Média ponderada por movimentações |
| Impacto | Investimento estadual obras (anual) | `siof_anual_empenhado`, `siof_anual_pago` | `siof.py` | Anual replicado em meses |
| Controle | Bolsa Família | `bf_valor_total`, `bf_beneficiarios` | `bolsa_familia_ce` | Inclui Auxílio Brasil e Novo BF |
| Controle | BPC | `bpc_valor_total`, `bpc_beneficiarios` | `bpc` | Por município, somado para a região |
| Controle | Transf. constitucionais federais | `transf_fed_fpm_dest`, `transf_fed_fundeb_dest`, `transf_fed_itr_dest`, `transf_fed_royalties_dest`, `transf_fed_outros_dest`, `transf_fed_total` | `transferencias_municipais` | CKAN STN |
| Controle | Transf. estaduais (cota-parte) | `transf_est_icms_dest`, `transf_est_ipva_dest`, etc. | `sefaz_ce` | Adapter — requer arquivo manual |
| Controle | Investimento federal voluntárias | `invest_fed_valor_total`, `invest_fed_n_transferencias` | `invest_federal` | Convênios e contratos de repasse |
| Controle | Investimento municipal | `invest_mun_valor` | `invest_municipal` | Adapter — planilha do Bruno |
| Controle | Crédito BNB (ESTBAN) | `bnb_credito_bnb_total`, `bnb_bnb_emprestimos_titulos_descontados`, `bnb_bnb_financiamentos`, etc. | `estban` | Adapter — verbetes COSIF 160-169 |
| Controle estadual | IBCR-CE | `ibcr_ce` | BACEN-SGS série 25380 (ajuste sazonal) | Mesmo valor para as 14 regiões |

### Fontes dos coletores CE

| Coletor | URL/origem | Granularidade | Defasagem |
|---------|------------|---------------|-----------|
| `caged_municipal` | FTP MTE/PDET | Município mensal | ~60 dias |
| `bolsa_familia_ce` | `portaldatransparencia.gov.br/download-de-dados/{novo-bolsa-familia\|auxilio-brasil\|bolsa-familia-pagamentos}/AAAAMM` | Município mensal | ~30 dias |
| `bpc` | `portaldatransparencia.gov.br/download-de-dados/bpc/AAAAMM` | Município mensal | ~30-60 dias |
| `transferencias_municipais` | `tesourotransparente.gov.br/ckan/...` (CKAN) | Município mensal | ~10 dias |
| `invest_federal` | `portaldatransparencia.gov.br/download-de-dados/transferencias/AAAAMM` (filtro Voluntárias) | Município mensal | ~30 dias |
| `estban` | Sistema Cosif BCB ou Base dos Dados | Município mensal | ~60 dias |
| `sefaz_ce` | Ceará Transparente (manual) | Município mensal | ~30 dias |
| `invest_municipal` | Planilha do Bruno | Município mensal/anual | — |

### Mapeamento município → região CE

`pipeline/regioes_ce.py` — 184 municípios CE × 14 regiões. Arquivo CSV em
`pipeline/data/municipios_ce_regioes.csv`. Códigos IBGE de 7 dígitos vêm da
API IBGE (`servicodados.ibge.gov.br/api/v1/localidades/estados/23/municipios`),
cacheada em `pipeline/data/municipios_ce_ibge_cache.csv`. Aliases ortográficos
(ex.: Itapagé → Itapajé) tratados em `_ALIASES_NOME`.

### Pendências regionais CE

| Item | Descrição | Impacto |
|------|-----------|---------|
| Investimento privado | Não implementado — aguardando definição do Prof. Paulo sobre fonte do "investimento total" (PIB FBCF? JUCEC?) | Controle privado regional ausente |
| ESTBAN download programático | BCB removeu URL pública estável; usar Sistema Cosif manual ou Base dos Dados via BigQuery | Crédito BNB depende de download manual |
| SEFAZ-CE auto-download | Ceará Transparente bloqueia requests automatizados (WAF) | ICMS/IPVA depende de download manual |
| Salário regional Paulo Ícaro | Cálculo do CAGED `salario_mensal_medio` por município ainda em construção pelo Paulo Ícaro | Coluna `salario_medio` pode precisar reconciliação posterior |
